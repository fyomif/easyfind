    ...
    <div className="text-left">
      {
        props.list[props.step - 1].items?.map((item, index) => {
          return (
            <SingleQuestion key={`${index}_${item.label}`} item={item} onChange={updateAnswers} answer={props.pagesAnswers[props.step] ? props.pagesAnswers[props.step][item.value] : null} />
          )
        })
      }
    </div>
